...
## 📊 Project Progress & Time Log
> Tracked using Toggl

### ⏳ Completed Tasks & Duration
✅ Codebase Analysis & Ideation (4 hours)
- Study **LLaVA model architecture** and multi-modal capabilities
- Analyze existing codebase organization and structure
- Review current documentation status
- Identify performance bottlenecks and areas for optimization

✅ Colab Setup & Training (3 hours)
...
- ✅ **Scale efficiently**
    - multi-host training support
    - multi-image support for interface
    - **Support multiple modalities** (PDF, video, audio)
- ✅ Collaboration
    - documentation
...
1. **Can the model handle long-context data?**
    - The model might face issues with longer input sequences.
    - **Flash Attention** is already applied, improving memory efficiency.
    - The model leverages **Rotary Positional Embeddings**, optimizing performance.
2. **Can we train the model with limited resources?**
    - **PEFT (Parameter-Efficient Fine-Tuning)** is already integrated, making fine-tuning efficient.
    - **Quantization techniques (4-bit, 8-bit) & low-rank adaptation** are used to reduce training costs.
3. **What happens if the model itself scales up?**
    - **DeepSpeed** is already applied, so multi-node scaling isn't a major issue.
    - 🚧 However, **the current implementation is limited to single-GPU training**.

#### ✅ Action

1. Modify **training scripts** to support **multi-host training** for better scalability.

### 📊 Data Scalability

#### 🔥 Ideation

1. **Can we efficiently manage and preprocess large datasets?**
    - The dataset pipeline lacks a **centralized management system**, making integration cumbersome.
    - 🚧 No built-in support for **dataset combination experiments**.
2. **Does the model properly handle `<Image>` token placement?**
    - 🚧  If users manually type `<Image>`, it may cause **positional misalignment issues**.
3. **Does the system support diverse prompts?**
    - Currently, the system uses **a fixed prompt template per model**.
4. **Does the model support multi-image inputs?**
    - 🚧 The model **supports multi-image processing**, but the **interface only accepts a single image**.
5. **Can the model handle multi-modality (PDF, Video, Audio, etc.)?**
    - **Currently, the model only supports images**.
    - Other modalities **(video, audio, PDF)** must be converted into images before processing.
...
1. Extend the **interface to accept multi-image inputs**.
2. Modify the **interface to support multi-modal inputs**.
    - **Converting video/audio to images** at the input stage is feasible, and a simple approach would be to handle this transformation directly in the **dataloader**, ensuring that all inputs are uniformly converted to images before being passed into the model.
    - However, for **better functionality**, each modality (video, audio) should ideally be **passed separately to the model**, allowing it to **extract meaningful features from each modality** before applying **modality-specific processing**.
    - This approach would increase **implementation complexity**, as it requires modifying **`llava_arch.py`'s `prepare_inputs_labels_for_multimodal` function** to:
        - Handle **image and video/audio separately**, ensuring each follows an appropriate preprocessing pipeline.
        - Introduce **modality-specific feature extraction** instead of treating all inputs as images.
    - Given the **limited time**, the current focus is on **simpler solutions**, such as:
        - **Video → Extracting key frames as images** in the dataloader.
        - **Audio → Converting spectrograms into images** in the dataloader.

### 🤝 Collaboration & Code Maintainability

#### 🔥 Ideation

1. **Is there a proper test suite?**
    - 🚧 **No test cases exist**, increasing the risk of introducing bugs when modifying code.
2. **Is there sufficient documentation?**
    - **Usage documentation exists**, but **developer-focused documentation is lacking**.
    - 🚧  No clear **data flow documentation**, making it difficult to understand how **data moves through the model**.
    - 🚧  **No docstrings for functions/classes**, making debugging and modifications harder.
3. **Is the code well-structured for collaboration?**
    - No **clear separation of modular components**, making **parallel development difficult**.
    - **Model-related functions are too large and contain multiple nested operations within a single function, making them difficult to read and debug.**
    - 🚧  **There is a lack of comments or explanations, making it difficult for developers to understand the code structure without extensive manual inspection.**

#### ✅ Action

1. Improve **developer documentation**
    - **Code structure overview**
    - **Data flow diagrams**
    - **Function & class docstrings**
2. Refactor the codebase
    - Design and separate **modular components** for easier parallel development.
    - **Break down large model-related functions into smaller, single-responsibility functions** for better readability and maintainability.
    - Add **inline comments and docstrings** to explain complex logic and enhance code clarity.
